...
type EditImage = (SelectedImage & { isExisting?: false }) | ExistingImage;

export default function EditListingScreen({
  route,
  navigation,
}: SellScreenProps<"EditListing">) {
...
function EditListingForm({
  ...
  listing: Listing;
  listingId: string;
  navigation: SellScreenProps<"EditListing">["navigation"];
}) {
  const initialImages: EditImage[] = (listing.listing_images ?? []).map(
    (img) => ({
      uri: img.image_url,
      id: img.id,
  ...
  const {
    ...
    defaultValues: {
      ...
      size: (listing.size as ListingFormData["size"]) ?? undefined,
  ...
  const pickImages = async () => {
    ...
    if (!result.canceled && result.assets.length > 0) {
      const newImages: EditImage[] = result.assets
  ...
  const updateMutation = useMutation({
    mutationFn: async (data: ListingFormData) => {
      ...
      setUploadProgress("Updating photos...");
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        if (img.isExisting) {
          ...
          const url = await uploadImage("listing-images", path, img.uri);
      ...
      if (imageUrls.length > 0) {
        await createListingImages(listingId, imageUrls);
      }
    },
    onSuccess: () => {
